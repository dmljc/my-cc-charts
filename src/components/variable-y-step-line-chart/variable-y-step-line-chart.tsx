...
/** 默认 x 轴测试数据（100 条） */
const DEFAULT_X_DATA = Array.from({ length: 100 }, (_, index) => `01-${String(index + 1).padStart(2, '0')}`);

/** 默认系列数据 */
const DEFAULT_SERIES: YAxisSeriesConfig[] = [
  {
    name: '曲线A',
    data: Array.from({ length: 100 }, (_, index) => Number((0.015 + index * 0.015).toFixed(3))),
  },
  {
    name: '曲线B',
    data: Array.from({ length: 100 }, (_, index) => Number((0.02 + ((index * 7) % 100) * 0.012).toFixed(3))),
  },
  {
    name: '曲线C',
    data: Array.from({ length: 100 }, (_, index) => Number((0.04 + Math.abs(Math.sin(index / 5)) * 0.28).toFixed(3))),
  },
  {
    name: '高值曲线',
    data: Array.from({ length: 100 }, (_, index) => Number((1 + index * 0.09).toFixed(2))),
  },
];
